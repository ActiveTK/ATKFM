<?php
  ...
  */
  ...
  if (isset($_GET["ajax-typeof"]) || isset($_GET["ajaxtypeof"]))
  {
    ...
    $hiki = "";
    if (isset($_GET["ajax-option"])) $hiki = htmlspecialchars($_GET["ajax-option"]);
    if (isset($_GET["ajaxoption"])) $hiki = htmlspecialchars($_GET["ajaxoption"]);
    if (isset($_GET["ajax-option2"])) $hiki2 = htmlspecialchars($_GET["ajax-option2"]);
    if (isset($_GET["ajaxoption2"])) $hiki2 = htmlspecialchars($_GET["ajax-option2"]);
    ...
    else if ($command == "uploadfromurl" || $command == "uploadfromurl")
    {
      ...
      $isd = file_get_contents("https://www.kaihi.cf/getonly.php?q=".base64_encode($hiki));
      if (substr($isd, 0, 10) == "cURL error") exit($isd);
      file_put_contents($filename, $isd);
...
<title><?=$title?></title>
<?php ViewActiveTKJS(); ?>
<script type="text/javascript">var starttitle="<?=$title?>";function save(){$.ajax({url:"<?=$meurl?>?ajax-typeof=save-item&ajax-option="+encodeURIComponent(_("pt").value),type:"post",data:{naka:_("naka").innerText},success:function(t){""==t?(_("info").innerHTML="変更を保存しました。",$("title").html(starttitle)):_("info").innerHTML="変更を保存出来ませんでした。"}}).fail(function(t,n,e){_("info").innerHTML="エラー:変更を保存出来ませんでした。詳細: "+e,alert("変更を保存出来ませんでした。")})}$(document).ready(function(){$("#naka").on("DOMSubtreeModified propertychange",function(){$("title").html("*"+starttitle)})});document.onkeydown=function(e){if(event.ctrlKey&&83==event.keyCode)return save(),event.keyCode=0,!1};function newfm(){window.open("<?=$meurl?>?ajax-typeof=get-item&ajax-option="+atk.encode(_("pt").value),"_blank")}</script>
</head>
<body style="background-color:#e6e6fa;text:#363636;">
<form action="<?=$meurl?>?ajax-type=get-item&ajax-option=<?php echo $filepath; ?>" method="POST" onsubmit="save();return false;">
  ...
  <font color="#e6e6fa">
    <pre>
      <div <?php if (!isset($_GET["readonly"])) echo "contenteditable "; ?>id="naka" style="text-align:left;position:fixed;overflow-wrap:break-word;overflow-x:scroll;overflow-y:visible;width:100%;height:80%;background-color:#000000;margin:5px 5px;font-size:19px;"><?php
  ...
  <br>
  <div style="position:fixed;bottom:3%;left:6;">
    <input type="submit" value="保存">
    <input type="button" value="クリア" onclick="_('naka').innerHTML='';">
    <input type="button" value="置き換え" onclick='let e=_("naka").innerText,n=window.prompt("置き換えるテキストを入力してください"),o=window.prompt("置き換え後のテキストを入力してください");if(n&&o){for(p=e.replace(n,o);p!==e;)e=e.replace(n,o),p=p.replace(n,o);_("naka").innerText=p}else;'>
    <input type="button" value="文字数取得" onclick="_('info').innerHTML='現在、'+_('naka').innerText.length+'文字です。';">
  </div>
</form>
</body>
</html>
