<?php // been updated automatically on 2021/07/03 - Jul (Sat) 22:42:36(UNIXTIME:1625319756) from https://www.activetk.cf/ATK-FM/update/go/1.1.2.php
  /////////////////////////////////////////////////
  define('fmversion', 'v1.1.2');
  define('lastupdateinfo', '2021/07/03 - Jul (Sat) 22:42:36');
...
<html lang="ja" dir="ltr">
  <head>
    ...
    function GetFileList(){"/"!=_("directory-root").value.substr(-1,1)&&(_("directory-root").value=_("directory-root").value+"/"),$.ajax({url:"<?=$meurl?>?ajax-typeof=get-directory&ajax-option="+encodeURIComponent(_("directory-root").value),type:"get",dataType:"json",success:function(e){if(_("server-data").innerHTML="",e["atk-fm-error"])_("info").innerHTML=e["atk-fm-error"];else for(let n in e){let t=e[n],o=n.slice(n.lastIndexOf("/")+1);_("server-data").innerHTML="a"==t?_("server-data").innerHTML+"<img src='"+fileimage+"' style='width:22px;height:22px;'> "+o+"<a href='<?=$meurl?>?ajax-typeof=get-item&ajax-option="+encodeURIComponent(n)+"' target='_blank' rel='noopener noreferrer'><input type='button' value='開く'></a><a href='javascript:edit(\""+n+"\");'><input type='button' value='操作'></a><br>":"b"==t?_("server-data").innerHTML+"<img src='"+ffimage+"' style='width:22px;height:22px;'>"+o+'(フォルダー)<a href=\'javascript:_("directory-root").value="'+n+"\";GetFileList();'><input type='button' value='開く'></a><a href='javascript:DirectoryRemove(\""+n+"\");'><input type='button' value='削除'></a><a href='javascript:MakeZip(\""+n+"\");'><input type='button' value='圧縮'></a><a href='javascript:RenameItem(\""+n+"\");'><input type='button' value='リネーム'></a><a href='javascript:CopyD(\""+n+"\");'><input type='button' value='コピー'></a><a href='javascript:CountDirectoryFiles(\""+n+"\");'><input type='button' value='詳細'></a><br>":"c"==t?_("server-data").innerHTML+"<img src='"+fzimage+"' style='width:22px;height:22px;'>"+o+'(zip)<a href=\'javascript:_("directory-root").value="'+n+"\";GetFileList();'><input type='button' value='プレビュー'></a><a href='javascript:OpenZip(\""+n+"\");'><input type='button' value='展開'></a><a href='javascript:FileRemove(\""+n+"\");'><input type='button' value='削除'></a><a href='<?=$meurl?>?ajax-typeof=download-item&ajax-option="+encodeURIComponent(n)+"'><input type='button' value='ダウンロード'></a><a href='javascript:RenameItem(\""+n+"\");'><input type='button' value='リネーム'></a><a href='javascript:CopyItem(\""+n+"\");'><input type='button' value='コピー'></a><a href='javascript:GetSize(\""+n+"\");'><input type='button' value='容量取得'></a><br>":"d"==t?_("server-data").innerHTML+"<img src='data:image/png;base64,' style='width:22px;height:22px;'> "+o+"(zip内ファイル)<a href='<?=$meurl?>?ajax-typeof=get-item-zip&ajax-option="+encodeURIComponent(_("directory-root").value)+"&ajax-option2="+encodeURIComponent(n)+"' target='_blank' rel='noopener noreferrer'><input type='button' value='表示'></a><a href='javascript:RemoveZip(\""+n+"\");'><input type='button' value='削除'></a><br>":_("server-data").innerHTML+"(未対応のファイル)<br>"}}}).fail(function(e,n,t){_("info").innerHTML="エラー:データを更新出来ませんでした。詳細 : "+t})}
