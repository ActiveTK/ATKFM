<?php // been updated automatically on 2021/06/30 - Jun (Wed) 20:38:43(UNIXTIME:1625053123) from https://www.activetk.cf/ATK-FM/update/go/1.0.7.php
  /////////////////////////////////////////////////
  define('fmversion', 'v1.0.7');
  define('lastupdateinfo', '2021/06/30 - Jun (Wed) 20:38:43');
  define('publishupdateinfo', '2021/06/19 - Jun (Sat) 13:29:47');
  define('copyright',
...
<html lang="ja" dir="ltr">
  ...
  <body style="background-color:#e6e6fa;text:#363636;">
    <div align="center">
      ...
      <pre><?=copyright?></pre>
      <p>最終アップデート実行時刻: <?=lastupdateinfo?> [<a href="<?=$_SERVER["REQUEST_URI"]?>&checkupdate=false">アップデートを確認</a>] [<a href="<?=$_SERVER["REQUEST_URI"]?>&checkupdate=true">強制アップデートを実行</a>]</p>
      <p>最終アップデート確認日時: <?=file_get_contents(dirname(__FILE__) . "/ATK-FM/lastupdate.txt")?> (今日:<?=date("Ymd")?>)</p>
      <pre>※アップデートの確認は毎日最初のアクセス時に行われます。</pre>
      <p>CPU(loadavg[0]): <?php echo sys_getloadavg()[0]; ?>%使用中</p>
      <p>Memory: <?php
        try {
      ...
      <p>Disk</p>
      <?php
      echo "約".$free_space ."Byte(".@round(($free_space/1024*1024),2)."MB、".@round(($free_space/$gb), 2) . "GB)利用可能<br>\n";
