<?php // been updated automatically on 2021/06/18 - Jun (Fri) 18:09:24(UNIXTIME:1624007364) from https://www.activetk.cf/ATK-FM/update/go/1.0.5.php
  /////////////////////////////////////////////////
  define('fmversion', 'v1.0.5');
  define('lastupdateinfo', '2021/06/18 - Jun (Fri) 18:09:24');
  define('publishupdateinfo', '2021/06/18 - Jun (Fri) 17:43:22 / by Github');
  /////////////////////////////////////////////////
  /*!
   * ATK-FM v1.0.5 | PHP/HTML/JS/CSS
  ...
  */
  ...
  function CheckUpdate($real) {
    if (lastupdateinfo == 'BETA!') return false;
    if (file_get_contents(dirname(__FILE__) . "/ATK-FM/lastupdate.txt") == date("Ymd") && $real != true) return false;
    $hasupdate = downloadtext("https://www.activetk.cf/ATK-FM/update/lastbuild");
    if ($hasupdate == fmversion && $real != true) return false;
  ...
  if (isset($_POST["setup"]) && isset($_POST["password"]))
  {
    ...
    else
    {
      ...
      file_put_contents(dirname(__FILE__) . "/ATK-FM/password.txt", $pw);
      file_put_contents(dirname(__FILE__) . "/ATK-FM/lastupdate.txt", date("Ymd"));
      $_SESSION["login"] = $pw;
  ...
  if (!File_Exists(dirname(__FILE__) . "/ATK-FM/password.txt")) ViewStartForm();
  if (isset($_SESSION["login"]) && $_SESSION['login'] == file_get_contents(dirname(__FILE__) . "/ATK-FM/password.txt")) {}
  else if (isset($_POST["login"]) && isset($_POST["password"]))
  {
    if (password_verify($_POST['password'], file_get_contents(dirname(__FILE__) . "/ATK-FM/password.txt")))
    {
      $_SESSION["login"] = file_get_contents(dirname(__FILE__) . "/ATK-FM/password.txt");
      $_SESSION["cd"] = realpath(null) . "/";
    }
    else
    {
      exit("<p>LoginError: パスワードが違います</p>");
    }
  }
  else ViewLoginForm();
  if (isset($_POST["save-password"]))
  {
    $newpas = password_hash($_POST['passwordnew'], PASSWORD_DEFAULT);
    if (password_verify($_POST['passwordold'], file_get_contents(dirname(__FILE__) . "/ATK-FM/password.txt")))
    {
      file_put_contents(dirname(__FILE__) . "/ATK-FM/password.txt", $newpas);
      $_SESSION["login"] = $newpas;
      exit("パスワードを保存しました。");
    }
  }
    ...
    else if ($command == "upload-item" || $command == "uploaditem")
    {
       for($i = 0; $i < count($_FILES["file"]["name"]); $i++ ){
          if(is_uploaded_file($_FILES["file"]["tmp_name"][$i])){
            ...
            if (move_uploaded_file($_FILES["file"]["tmp_name"][$i], $filename))echo $_FILES["file"]["name"][$i] . "をアップロードしました。";
            else echo $_FILES["file"]["name"][$i] . "をアップロード出来ませんでした。";
          }
        }
    }
  ...
  <body style="background-color:#e6e6fa;text:#363636;">
    <div align="center">
      ...
      <form action='<?=$meurl?>' method='POST'>
        <p>パスワード: <input type="password" name="password" placeholder="パスワードを入力してください" required></p>
        <input type="submit" name="login" value="ログイン" title="ログイン">
      </form>
    </div>
  </body>
</html>
...
<head>
    ...
    function MakeZip(ed){$.ajax({url:"<?=$meurl?>?ajax-typeof=make-zip&ajax-option="+encodeURIComponent(ed),type:"get",success:function(e){_("info").innerHTML=e,GetFileList()}}).fail(function(e,n,t){_("info").innerHTML="エラー:zip圧縮出来ませんでした。詳細 : "+t})}function CloneItem(){$.ajax({url:"<?=$meurl?>?ajax-typeof=uploadfromurl&ajax-option="+encodeURIComponent(_("clone-url").value),type:"get",success:function(e){_("info").innerHTML=e,GetFileList()}}).fail(function(e,n,t){_("info").innerHTML="エラー:クローン出来ませんでした。詳細 : "+t})}
    ...
    function closep(){_("selecteditem").value="",_("pp").style="display:none;text-align:center;position:fixed;bottom:10%;left:10%;height:80%;width:80%;background-color:#00ff7f;color:#080808;overflow-x:hidden;overflow-y:visible;",_("run-file").style="display:none;",_("run-fl").style="display:none;"}
    function GetFileList(){
    "/"!=_("directory-root").value.substr(-1,1)&&(_("directory-root").value=_("directory-root").value+"/"),$.ajax({url:"<?=$meurl?>?ajax-typeof=get-directory&ajax-option="+encodeURIComponent(_("directory-root").value),type:"get",dataType:"json",success:function(e){if(_("server-data").innerHTML="",e["atk-fm-error"])_("info").innerHTML=e["atk-fm-error"];else for(let n in e){let t=e[n],o=n.slice(n.lastIndexOf("/")+1);
    
    _("server-data").innerHTML="a"==t?_("server-data").innerHTML+"<img src='"+fileimage+"' style='width:22px;height:22px;'> "+o+

    "<a href='<?=$meurl?>?ajax-typeof=get-item&ajax-option="+encodeURIComponent(n)+
    "' target='_blank' rel='noopener noreferrer'><input type='button' value='開く'></a><a href='javascript:edit(\""+n+"\");'><input type='button' value='操作'></a><br>"
    
    :"b"==t?_("server-data").innerHTML+"<img src='"+ffimage+"' style='width:22px;height:22px;'>"+o+
'(フォルダー)<a href=\'javascript:_("directory-root").value="'+n+"\";GetFileList();'><input type='button' value='開く'></a><a href='javascript:DirectoryRemove(\""+n+"\");'><input type='button' value='削除'></a><a href='javascript:MakeZip(\""+n+"\");'><input type='button' value='圧縮'></a><a href='javascript:RenameItem(\""+n+"\");'><input type='button' value='リネーム'></a><a href='javascript:CopyD(\""+n+"\");'><input type='button' value='コピー'></a><a href='javascript:CountDirectoryFiles(\""+n+"\");'><input type='button' value='詳細'></a><br>":"c"==t?_("server-data").innerHTML+"<img src='data:image/png;base64,' style='width:22px;height:22px;'>"+o+'(zip)<a href=\'javascript:_("directory-root").value="'+n+"\";GetFileList();'><input type='button' value='プレビュー'></a><a href='javascript:OpenZip(\""+n+"\");'><input type='button' value='展開'></a><a href='javascript:FileRemove(\""+n+"\");'><input type='button' value='削除'></a><a href='<?=$meurl?>?ajax-typeof=download-item&ajax-option="+encodeURIComponent(n)+"'><input type='button' value='ダウンロード'></a><a href='javascript:RenameItem(\""+n+"\");'><input type='button' value='リネーム'></a><a href='javascript:CopyItem(\""+n+"\");'><input type='button' value='コピー'></a><a href='javascript:GetSize(\""+n+"\");'><input type='button' value='容量取得'></a><br>":"d"==t?_("server-data").innerHTML+"<img src='data:image/png;base64,' style='width:22px;height:22px;'> "+o+"(zip内ファイル)<a href='<?=$meurl?>?ajax-typeof=get-item-zip&ajax-option="+encodeURIComponent(_("directory-root").value)+"&ajax-option2="+encodeURIComponent(n)+"' target='_blank' rel='noopener noreferrer'><input type='button' value='表示'></a><a href='javascript:RemoveZip(\""+n+"\");'><input type='button' value='削除'></a><br>":_("server-data").innerHTML+"(未対応のファイル)<br>"}}}).fail(function(e,n,t){_("info").innerHTML="エラー:データを更新出来ませんでした。詳細 : "+t})}function DirectoryRemove(e){confirm("本当にディレクトリを削除しますか？")&&$.ajax({url:"<?=$meurl?>?ajax-typeof=remove-directory&ajax-option="+encodeURIComponent(e),type:"get",success:function(e){_("info").innerHTML="ディレクトリを削除しました。",GetFileList()}}).fail(function(e,n,t){_("info").innerHTML="エラー:ディレクトリを削除出来ませんでした。詳細 : "+t})}function FileRemove(e){confirm("本当にファイルを削除しますか？")&&$.ajax({url:"<?=$meurl?>?ajax-typeof=remove-item&ajax-option="+encodeURIComponent(e),type:"get",success:function(e){_("info").innerHTML="ファイルを削除しました。",GetFileList()}}).fail(function(e,n,t){_("info").innerHTML="エラー:ファイルを削除出来ませんでした。詳細 : "+t})}function DownloadFile(e){}function AddItem(){$.ajax({url:"<?=$meurl?>?ajax-typeof=add-item&ajax-option="+encodeURIComponent(_("add-item-name").value),type:"get",success:function(e){_("info").innerHTML="ファイルを新規作成しました。",GetFileList()}}).fail(function(e,n,t){_("info").innerHTML="エラー:ファイルを作成出来ませんでした。詳細 : "+t})}function UploadFile(){_("info").innerHTML="ファイルをアップロードしています。。(この処理には数分間かかる場合があります)",$.ajax({url:"<?=$meurl?>?ajax-typeof=upload-item",type:"POST",data:new FormData($("#upload-file").get(0)),cache:!1,contentType:!1,processData:!1,dataType:"html"}).done(function(e){""==e?(_("info").innerHTML="ファイルをアップロードしました。",_("file").value="",GetFileList()):(_("info").innerHTML=e,GetFileList())}).fail(function(e,n,t){_("info").innerHTML="エラー:ファイルをアップロード出来ませんでした。詳細 : "+t})}function RenameItem(e,n){if(!(n=prompt("パスを入力してください",e)))return!1;$.ajax({url:"<?=$meurl?>?ajax-typeof=rename-item&ajax-option="+encodeURIComponent(e)+"&ajax-option2="+encodeURIComponent(n),type:"get",success:function(e){_("info").innerHTML="ファイル名を変更しました。",GetFileList()}}).fail(function(e,n,t){_("info").innerHTML="エラー:ファイル名を変更出来ませんでした。詳細 : "+t})}function CopyItem(e){_("info").innerHTML="ファイルをコピーしています。。",$.ajax({url:"<?=$meurl?>?ajax-typeof=copy-item&ajax-option="+encodeURIComponent(e),type:"get",success:function(e){_("info").innerHTML="ファイルをコピーしました。",GetFileList()}}).fail(function(e,n,t){_("info").innerHTML="エラー:ファイルをコピー出来ませんでした。詳細 : "+t})}function AddDirectory(){"/"!=_("directory-root").value.substr(-1,1)&&(_("add-directory-name").value=_("add-directory-name").value+"/"),$.ajax({url:"<?=$meurl?>?ajax-typeof=add-directory&ajax-option="+encodeURIComponent(_("add-directory-name").value),type:"get",success:function(e){_("info").innerHTML="ディレクトリを新規作成しました。",GetFileList()}}).fail(function(e,n,t){_("info").innerHTML="エラー:ディレクトリを作成出来ませんでした。詳細 : "+t})}function CountDirectoryFiles(e){_("info").innerHTML="詳細情報を取得しています。。",$.ajax({url:"<?=$meurl?>?ajax-typeof=count-directory-files&ajax-option="+encodeURIComponent(e),type:"get",dataType:"json",success:function(e){_("info").innerHTML=e.File+"個のファイル、"+e.Directory+"個のディレクトリがあります。計測時間:"+e.Time+"s、サイズ:"+e.SIZE,GetFileList()}}).fail(function(e,n,t){_("info").innerHTML="エラー:詳細情報を取得出来ませんでした。詳細 : "+t})}function GetSize(e){_("info").innerHTML="ファイルのサイズを取得しています。。",$.ajax({url:"<?=$meurl?>?ajax-typeof=get-filesize&ajax-option="+encodeURIComponent(e),type:"get",success:function(e){_("info").innerHTML="ファイルのサイズ: "+e}}).fail(function(e,n,t){_("info").innerHTML="エラー:ファイルのサイズを取得出来ませんでした。詳細 : "+t})}function OpenZip(e){_("info").innerHTML="展開しています。。",$.ajax({url:"<?=$meurl?>?ajax-typeof=open-zip&ajax-option="+encodeURIComponent(e),type:"get",success:function(e){_("info").innerHTML=e,GetFileList()}}).fail(function(e,n,t){_("info").innerHTML="エラー:ファイルを展開出来ませんでした。詳細 : "+t})}function RemoveZip(e){confirm("本当にファイルを削除しますか？")&&(_("info").innerHTML="削除しています。。",$.ajax({url:"<?=$meurl?>?ajax-typeof=remove-item-zip&ajax-option="+encodeURIComponent(_("directory-root").value)+"&ajax-option2="+encodeURIComponent(e),type:"get",success:function(e){_("info").innerHTML=e,GetFileList()}}).fail(function(e,n,t){_("info").innerHTML="エラー:ファイルを削除出来ませんでした。詳細 : "+t}))}function CopyD(e){_("info").innerHTML="ディレクトリをコピーしています。。",$.ajax({url:"<?=$meurl?>?ajax-typeof=copy-item&ajax-option="+encodeURIComponent(e),type:"get",success:function(e){_("info").innerHTML="ディレクトリをコピーしました。",GetFileList()}}).fail(function(e,n,t){_("info").innerHTML="エラー:ディレクトリをコピー出来ませんでした。詳細 : "+t})}onload=function(){GetFileList()};  
    </script>
    <style>a{color: #00ff00;position: relative;display: inline-block;transition: .3s;}a::after {position: absolute;bottom: 0;left: 50%;content: '';width: 0;height: 2px;background-color: #31aae2;transition: .3s;transform: translateX(-50%);}a:hover::after{width: 100%;}</style>
  </head>
  <body style="background-color:#e6e6fa;text:#363636;">
    ...
    <div id="pp" style="display:none;text-align:center;position:fixed;bottom:10%;left:10%;height:80%;width:80%;background-color:#00ff7f;color:#080808;overflow-x:hidden;overflow-y:visible;">
      ...
      <div id="options">
        ...
        <div id="run-fl" style="display:none;">
...
<?php function ViewNotePad($filepath){
$s = mb_strstr($_SERVER["REQUEST_URI"], '?', true);
if ($s == "") $s = $_SERVER["REQUEST_URI"];
$meurl = (((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ) ? "https://" : "http://").$_SERVER['HTTP_HOST'].$s;
...
<html>
  <head>
    ...
    <script type="text/javascript">var starttitle="<?=$title?>";function save(){$.ajax({url:"<?=$meurl?>?ajax-typeof=save-item&ajax-option="+encodeURIComponent(_("pt").value),type:"post",data:{naka:_("naka").value},success:function(t){""==t?(_("info").innerHTML="変更を保存しました。",$("title").html(starttitle)):_("info").innerHTML="変更を保存出来ませんでした。"}}).fail(function(t,n,e){_("info").innerHTML="エラー:変更を保存出来ませんでした。詳細: "+e,alert("変更を保存出来ませんでした。")})}$(document).ready(function(){$("#naka").on("change",function(){$("title").html("*"+starttitle)})});document.onkeydown=function(e){if(event.ctrlKey&&83==event.keyCode)return save(),event.keyCode=0,!1};function newfm(){window.open("<?=$meurl?>?ajax-typeof=get-item&ajax-option="+atk.encode(_("pt").value),"_blank")}</script>
    <?php } ?>
...
<p>ATK-FM <?=fmversion?> (公開時刻:<?=publishupdateinfo?>)</p>
<p>最終アップデート: <?=lastupdateinfo?> [<a href="<?=$_SERVER["REQUEST_URI"]?>&checkupdate=false">アップデートを確認</a>] [<a href="<?=$_SERVER["REQUEST_URI"]?>&checkupdate=true">強制アップデートを実行</a>]</p>
<p>CPU: <?php echo sys_getloadavg()[0]; ?>%使用中</p>
<p>Memory: <?php
...
echo "1分の平均負荷: ".$loads[0]."<br>\n";
echo "5分の平均負荷: ".$loads[1]."<br>\n";
echo "15分の平均負荷: ".$loads[2]."<br>\n";
?>
<p>Disk</p>
<?php
$mb = 1024 * 1024;
...
echo "約" . $free_space  . "Byte(".round( ($free_space / $mb), 2) . "MB、".round( ($free_space / $gb), 2) . "GB)利用可能<br>\n";
